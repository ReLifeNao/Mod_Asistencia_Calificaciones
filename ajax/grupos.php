...
switch ($_GET["op"]) {
	...
	case 'anular':
		...
		echo '<script>';
		echo 'console.log('. "SAMASSASDKVCNAOPN" .')';
		echo 'console.log('. $idgrupo .')';
		echo '</script>';
    ...
    case 'listar':
		...
		while ($reg=$rspta->fetch_object()) {
			...
			"0"=>'<button class="btn btn-warning btn-xs" onclick="mostrar('.$reg->idsalon.')"><i class="fa fa-pencil"></i></button>'.' '.'<button class="btn btn-danger btn-xs" onclick="desactivar('.$reg->idsalon.')"><i class="fa fa-close"></i></button>',
            "1"=>$reg->nombre,
            "2"=>$reg->usuario
              );
		}
