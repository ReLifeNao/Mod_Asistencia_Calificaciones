...
public function cantidadalumnos($user_id){
	$sql="SELECT COUNT(*) total_alumnos FROM alumno WHERE user_id='$user_id'";
	

	return ejecutarConsulta($sql);
}
public function cantidadalumnos_porgrupo($user_id,$idgrupo){
	$sql="SELECT a.id as idalumno, a.name,a.lastname,a.image  FROM alumno a INNER JOIN salon_alumno alt ON a.id=alt.alumn_id WHERE a.user_id='$user_id' AND alt.salon_id='$idgrupo'";
	
	
	return ejecutarConsulta($sql); 
}
public function cantidadg($user_id,$idgrupo){
	$sql="SELECT COUNT(*) total_alumnos  FROM alumno a INNER JOIN salon_alumno alt ON a.id=alt.alumn_id WHERE a.user_id='$user_id' AND alt.salon_id='$idgrupo'";
...
public function cantidadgrupos($idusuario){
	$sql="SELECT idsalon,nombre, idusuario, favorito FROM salones WHERE idusuario='$idusuario'";
	return ejecutarConsulta($sql);
}
...
public function cantidadgruposAdmin($idusuario){
	$sql="SELECT idsalon,nombre, idusuario, favorito FROM salones";
	return ejecutarConsulta($sql);
}
...
public function cantidadCursos($idusuario){
	$sql="SELECT * FROM salon_alumno inner join salones on salon_alumno.salon_id=salones.idsalon inner join cursos on salones.idsalon=cursos.salon_id WHERE salon_alumno.alumn_id='$idusuario'";
	return ejecutarConsulta($sql);
}	
