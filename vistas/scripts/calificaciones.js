...
function desactivar(idalumno){
	...
	var idcali= $("#idcurso").val();
	
	
	
	bootbox.confirm("¿Esta seguro de eliminar la nota?", function(result){
		if (result) {
			$.post("../ajax/calificaciones.php?op=desactivar", {valor:idcali, alumn_id:idalumno}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
